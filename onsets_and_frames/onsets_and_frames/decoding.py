...
def extract_notes(onsets, frames, onset_threshold=0.5, frame_threshold=0.5):
    ...
    """
    # new_onsets = torch.zeros(onsets.shape)
    # new_frames = torch.zeros(frames.shape)
    # for i, (o, maxarg) in enumerate(zip(onsets, torch.argmax(onsets, dim=-1))):
    #     temp = torch.zeros(o.shape)
    #     temp[maxarg] = o[maxarg]
    #     new_onsets[i] = temp
    # for i, (o, maxval) in enumerate(zip(frames, torch.argmax(frames, dim=-1))):
    #     temp = torch.zeros(o.shape)
    #     temp[maxarg] = o[maxarg]
    #     new_frames[i] = temp
    # onsets = new_onsets
