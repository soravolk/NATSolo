...
def evaluate(data, model, onset_threshold=0.5, frame_threshold=0.5, save_path=None):
    ...
    for label in data:
        ...
        macro_onset_label.extend(label['onset'].tolist())
        macro_frame_label.extend(label['frame'].tolist())
        macro_onset_pred.extend(pred['onset'].tolist())
        macro_frame_pred.extend(pred['frame'].tolist())
        ...
        p_ref = np.array([midi_to_hz(LOGIC_MIDI + midi) for midi in p_ref])
        i_est = (i_est * scaling).reshape(-1, 2)
        p_est = np.array([midi_to_hz(LOGIC_MIDI + midi) for midi in p_est])

        t_ref = t_ref.astype(np.float64) * scaling
        f_ref = [np.array([midi_to_hz(LOGIC_MIDI + midi) for midi in freqs]) for freqs in f_ref]
        t_est = t_est.astype(np.float64) * scaling
        f_est = [np.array([midi_to_hz(LOGIC_MIDI + midi) for midi in freqs]) for freqs in f_est]
    ...
    macro_onset_label = torch.tensor(macro_onset_label)
    macro_frame_label = torch.tensor(macro_frame_label)
    macro_onset_pred = torch.tensor(macro_onset_pred)
    macro_frame_pred = torch.tensor(macro_frame_pred)
    p_ref, i_ref = extract_notes(macro_onset_label, macro_frame_label)
    p_est, i_est = extract_notes(macro_onset_pred, macro_frame_pred, onset_threshold, frame_threshold)
    t_ref, f_ref = notes_to_frames(p_ref, i_ref, macro_frame_label.shape)
    t_est, f_est = notes_to_frames(p_est, i_est, macro_frame_pred.shape)
    i_ref = (i_ref * scaling).reshape(-1, 2)
    p_ref = np.array([midi_to_hz(LOGIC_MIDI + midi) for midi in p_ref])
    i_est = (i_est * scaling).reshape(-1, 2)
    p_est = np.array([midi_to_hz(LOGIC_MIDI + midi) for midi in p_est])
    ...
    metrics['metric/note/overlap_macro'].append(o)
    frame_metrics = evaluate_frames(t_ref, f_ref, t_est, f_est)
    metrics['metric/frame/f1_macro'].append(hmean([frame_metrics['Precision'] + eps, frame_metrics['Recall'] + eps]) - eps)

    for key, loss in frame_metrics.items():
        metrics['metric/frame/' + key.lower().replace(' ', '_') + '_macro'].append(loss)
    return metrics
