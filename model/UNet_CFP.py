...
class Encoder(nn.Module):
    def __init__(self,ds_ksize, ds_stride):
        ...
        self.block1 = block(9,16,(3,3),(1,1),ds_ksize, ds_stride)
...
class UNet_CFP(nn.Module):
    def __init__(self, ds_ksize, ds_stride, log=True, reconstruction=True, mode='imagewise', spec='Mel', device='cpu', XI=1e-6, eps=1e-2):
        super().__init__()
...
#             pianoroll2 = torch.sigmoid(self.linear1(x)) # Use the full LSTM output
    ...
    def run_on_batch(self, batch, batch_ul=None, VAT=False, frame_loader=None):
      device = self.device
    #   audio = batch['audio']
      ...
      if self.reconstruction:
          for step, (idx, feat) in enumerate(frame_loader):
              ...
              """   feat = (B, 9, h, window_size)
                    ...
                    reconstruct, pred1, pred2 = self(feat)
                    loss = criterion(pred1, tech) + criterion(pred2, tech)
               """
